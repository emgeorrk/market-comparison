...
class MonthlyDataTests(unittest.TestCase):
    ...
    def test_dashboard_can_export_and_share_visible_series_as_png(self) -> None:
        ...
        controls_start = html.index('<section class="controls" aria-label="Параметры графика">')
        share_button = html.index('id="share-button"')
        chart_section = html.index('<section aria-labelledby="chart-title">')
        self.assertLess(controls_start, share_button)
        self.assertLess(share_button, chart_section)
